...
<head>
    ...
    <title>Sua de thi: <?php echo e($quiz->title); ?></title>
...
<body class="bg-white" data-auth-required="true">
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 class="text-4xl font-bold text-gray-900 mb-8">Sua cau hoi trong de thi</h1>

        <?php if($errors->any()): ?>
            <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                <h3 class="font-semibold text-red-800 mb-2">Co loi xay ra:</h3>
        ...
        <div class="card-lg p-6 mb-8">
            <div class="mb-4">
                <h2 class="text-xl font-bold text-gray-900 mb-2">Nhap tu file</h2>
                <p class="text-gray-600 text-sm mb-4">Chon file .txt de them nhanh cau hoi. Quy tac dinh dang: Cau hoi tren 1 dong, dap an dung bat dau bang (*), dap an sai bat dau bang (-).</p>
            </div>
            <div id="import-area" class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
                ...
                <p class="text-gray-700 font-medium mb-1">Bam de chon file</p>
                <p class="text-gray-500 text-sm">hoac keo tha file vao day</p>
            </div>
            <input type="file" id="fileInput" accept=".txt" style="display: none;" onchange="handleFileUpload(this)">
        </div>

        <form action="<?php echo e(route('quiz.update', $quiz->id)); ?>" method="POST" id="quizForm" class="space-y-6">
            <?php echo csrf_field(); ?>
            <?php echo method_field('PUT'); ?>

            <div class="card-lg p-6">
                <h2 class="text-xl font-bold text-gray-900 mb-4">Thong tin bai thi</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Ten bai thi *</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Thoi gian (phut) *</label>
                        <input type="number" name="minutes" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                               value="<?php echo e(old('minutes', $quiz->minutes)); ?>" min="1" required>
                    </div>
                </div>
            </div>
    ...
    <script>
        const existingQuestions = <?php echo json_encode($editableQuestions, 15, 512) ?>;
        ...
        function addQuestion(content = '', options = []) {
                ...
                <div class="flex items-start gap-3 mb-3">
                    ...
                    <div class="flex-1">
                        <input type="text" name="questions[${qIndex}][options][${oIndex}]"
                               ...
                               value="${escapeHtml(opt.text)}" placeholder="Dap an ${String.fromCharCode(65 + oIndex)}" required>
                    </div>
                </div>`;
            });
            ...
            <div class="card-lg p-6 border-l-4 border-blue-500" id="q-${qIndex}">
                <div class="flex justify-between items-start mb-4">
                    <h3 class="text-lg font-bold text-gray-900 question-order">Cau hoi so ${qIndex + 1}</h3>
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Noi dung cau hoi *</label>
                    <input type="text" name="questions[${qIndex}][question_text]"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                           value="${escapeHtml(content)}" placeholder="Nhap noi dung cau hoi..." required>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-3">Chon dap an dung *</label>
                    <div class="space-y-2">
                        ${optionsHTML}
                    </div>
                </div>
        ...
        function removeQuestion(questionIndex) {
            ...
            if (document.querySelectorAll('#questions-container > .card-lg').length <= 1) {
                alert('De thi phai co it nhat 1 cau hoi.');
                return;
            }

            if (!confirm('Ban co chac chan muon xoa cau hoi nay?')) {
                return;
            }
        ...
        function reindexQuestionLabels() {
            ...
            labels.forEach((label, index) => {
                label.textContent = `Cau hoi so ${index + 1}`;
            });
        }
        ...
        function parseAndPopulate(text) {
            ...
            alert('Them cau hoi tu file thanh cong.');
        }
...
<?php /**PATH C:\Users\0pham\Documents\GitHub\multiplechoice\resources\views\quiz\edit.blade.php ENDPATH**/ ?>